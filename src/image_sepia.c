...
static const float uint8_to_float[] = {
...
static void sepia_one( struct pixel* const pixel ) {
    ...
    pixel->r = sat(
            uint8_to_float[old.r] * c[0][0] + uint8_to_float[old.g] * c[0][1] + uint8_to_float[old.b] * c[0][2]
    );
    pixel->g = sat(
            uint8_to_float[old.r] * c[1][0] + uint8_to_float[old.g] * c[1][1] + uint8_to_float[old.b] * c[1][2]
    );
    pixel->b = sat(
            uint8_to_float[old.r] * c[2][0] + uint8_to_float[old.g] * c[2][1] + uint8_to_float[old.b] * c[2][2]
    );
}
...
void sepia_inplace(struct image* img) {
    ...
    for (size_t i = 0; i < data_size - data_size % 4; i += 4) {
        float p[12] = {uint8_to_float[img->data[i].b], uint8_to_float[img->data[i+1].b], uint8_to_float[img->data[i+2].b],
                   uint8_to_float[img->data[i+3].b], uint8_to_float[img->data[i].g], uint8_to_float[img->data[i+1].g],
                   uint8_to_float[img->data[i+2].g], uint8_to_float[img->data[i+3].g], uint8_to_float[img->data[i].r],
                   uint8_to_float[img->data[i+1].r], uint8_to_float[img->data[i+2].r], uint8_to_float[img->data[i+3].r]};

        sepia_four(p, img->data + i);
    }

    for (size_t i = data_size - data_size % 4; i < data_size; ++i) {
